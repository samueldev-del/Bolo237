...
> = {
  inbox: {
    ...
    listLabel: "Inbox",
    emptyTitle: "La boite de reception est vide.",
    emptyDescription: "Les nouveaux emails Hostinger apparaitront ici apres synchronisation.",
    countLabel: "Messages a traiter",
  },
  archive: {
    title: "Archives Hostinger",
    description: "Consulte les emails archives depuis le dashboard admin sans repolluer la reception.",
    listLabel: "Archives",
    emptyTitle: "Aucun email archive.",
    emptyDescription: "Les messages archives depuis l'admin apparaitront ici.",
    countLabel: "Messages conserves",
  },
  trash: {
    title: "Corbeille Hostinger",
    description: "Visualise les messages deplaces dans la corbeille Hostinger depuis l'administration.",
    listLabel: "Corbeille",
    emptyTitle: "La corbeille est vide.",
    emptyDescription: "Les messages supprimes depuis l'admin apparaitront ici.",
    countLabel: "Messages supprimes",
  },
};
...
type MailboxActionKey = "archive" | "trash";

type MailboxActionConfig = {
  key: MailboxActionKey;
  label: string;
...
function getMailboxActions(view: AdminMailboxView): MailboxActionConfig[] {
  if (view === "inbox") {
    return [
      {
        key: "archive",
        ...
        Icon: Archive,
        className:
          "inline-flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm font-semibold text-zinc-800 transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60",
      },
      {
        key: "trash",
        label: "Supprimer",
        ...
        Icon: Trash2,
        className:
          "inline-flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60",
      },
    ];
  }
  ...
  if (view === "archive") {
    return [
      {
        key: "trash",
        label: "Envoyer a la corbeille",
        confirmMessage:
          "Ce message sera deplace des archives vers la corbeille Hostinger. Continuer ?",
        Icon: Trash2,
        className:
          "inline-flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60",
      },
...
export function AdminMailboxView({ view }: { view: AdminMailboxView }) {
  const meta = mailboxMeta[view];
  const {
    hydrateSnapshot,
  ...
  const [messageAction, setMessageAction] = useState<MailboxActionKey | null>(null);
  ...
  const mailboxActions = getMailboxActions(view);
  ...
  const loadInbox = useCallback(
    async (options: { force?: boolean; silent?: boolean; notify?: boolean } = {}) => {
      ...
      try {
        const snapshot = await fetchAdminInbox({ force: options.force, limit: 120, view });
        ...
        setNotice({
          tone: "error",
          message: error instanceof Error ? error.message : "Impossible de charger cette boite mail.",
        });
      } finally {
        if (options.silent) {
          setRefreshing(false);
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      void loadInbox({ silent: true, notify: view === "inbox" });
    }, 75_000);
  ...
  async function handleSelectEmail(email: AdminEmail) {
    ...
    try {
      await markAdminEmailAsRead(email.id);
      await loadInbox({ silent: true, notify: false });
    } catch (error) {
      setNotice({
        tone: "error",
  ...
  async function handleMailboxAction(action: MailboxActionConfig) {
    if (!selectedEmail) {
      return;
    }
          ...
          {mailboxTabs.map((tab) => (
            <Link
              ...
              className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${
                tab.view === view
                  ? "bg-[#8B4332] text-white shadow-sm"
                  : "border border-zinc-200 bg-white text-zinc-700 hover:border-[#DA7756] hover:text-[#8B4332]"
              }`}
            >
              {tab.label}
            </Link>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto]">
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Messages</p>
              <p className="mt-3 text-3xl font-semibold text-zinc-900">{summary?.totalCount ?? emails.length}</p>
              <p className="mt-1 text-sm text-zinc-500">{meta.countLabel}</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-sm text-zinc-500">Messages encore ouverts</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Dossier IMAP</p>
              <p className="mt-3 break-words text-lg font-semibold text-zinc-900">{sync?.mailbox ?? "Inconnu"}</p>
          ...
          <div className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:flex-row lg:flex-col">
            ...
            {view === "inbox" ? (
              ...
              <div className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-xs font-medium text-zinc-500">
                Les notifications navigateur restent reservees a la reception.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid min-h-[640px] gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
          <div className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
            <div className="border-b border-zinc-200 px-4 py-4">
              <p className="text-sm font-semibold text-zinc-900">{meta.listLabel}</p>
            ...
            {loading ? (
              ...
              <div className="flex min-h-[480px] flex-col items-center justify-center px-6 text-center">
                ...
                <p className="mt-3 text-sm font-medium text-zinc-700">{meta.emptyTitle}</p>
                <p className="mt-1 text-sm text-zinc-500">{meta.emptyDescription}</p>
              </div>
            ) : (
                  ...
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 text-xs font-medium uppercase tracking-[0.16em] text-zinc-400">
                        Dossier: {selectedEmail.mailboxPath}
                      </p>
                    </div>
                      ...
                      {mailboxActions.map((action) => {
                        ...
                        return (
                          <button
                            key={action.key}
                            type="button"
                            onClick={() => void handleMailboxAction(action)}
                            disabled={Boolean(messageAction) || sendingReply}
                            className={action.className}
                          >
                            {messageAction === action.key ? (
