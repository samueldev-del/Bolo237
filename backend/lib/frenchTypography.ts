...
function normalizeApostrophes(text: string) {
  // Apostrophe typographique systématique.
  return text.replace(/'/g, "’");
}

function normalizeFrenchPunctuationSpacing(text: string) {
  return text
    // Space before colon in French.
    .replace(/\s*:/g, `${NBSP}:`)
    // Narrow non-breaking spaces before ; ! ?
    .replace(/\s*;/g, `${NNBSP};`)
    .replace(/\s*!/g, `${NNBSP}!`)
    .replace(/\s*\?/g, `${NNBSP}?`);
}

function normalizeUppercaseAccents(text: string) {
  return text
    // Common high-value fixes requested by style guide.
...
function normalizeFrenchTypography(text: string) {
...
function normalizeBulletLists(text: string) {
  return String(text || "")
    .replace(/^\s*[•●▪◦]\s*/gm, "- ")
    .replace(/^\s*[-*]\s*/gm, "- ")
